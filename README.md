...
Статус: **0.7.1** — personal build. Подписан стабильным локальным
self-signed сертификатом (`whisper-hot-local`), не нотаризован,
собран одним разработчиком. Главное окно с Dashboard / History /
Settings / Setup, быстрый menu bar controller, пять стилей индикатора
записи (включая Studio в стиле SuperWhisper), контекстный роутинг по
...
из спящего режима. Keychain ACL repair: после первого `Разрешить всегда`
на старых ключах следующие сборки с тем же `whisper-hot-local`
сертификатом не должны снова просить доступ.
