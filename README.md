...
Статус: **0.6.5** — personal build. Подписан стабильным локальным
self-signed сертификатом (`whisper-hot-local`), не нотаризован,
собран одним разработчиком. Пять стилей индикатора записи (включая
новый Studio в стиле SuperWhisper), контекстный роутинг по активному
приложению, мульти-провайдер пост-обработка, локальный LLM cleanup,
технический словарь и офлайн-фоллбек на whisper.cpp.
