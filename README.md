...
Локальное macOS-приложение для speech-to-text с полноценным главным окном
и быстрым контроллером в строке меню. WhisperHot записывает твой голос,
...
Статус: **0.8.1** — personal build. Подписан сертификатом Developer ID
Application (Aleksei Supilin) и **нотаризован Apple**,
собран одним разработчиком. Главное окно с Dashboard / History /
Settings / Setup, быстрый menu bar controller, три стиля индикатора
записи (минимальный / средний / большой), контекстный роутинг по
активному приложению, мульти-провайдер пост-обработка, локальный LLM
cleanup, технический словарь, офлайн-фоллбек на whisper.cpp и
опциональное авто-переключение на offline при медленной сети.
Sleep/wake recovery: приложение больше не виснет после возврата Mac'а
из спящего режима. После `0.7.2` главное окно не читает API keys при
запуске, чтобы не провоцировать Keychain prompt-loop.

## Платформы

Сейчас shipped-версия поддерживает только macOS 13.0+ на Apple Silicon.
Windows-поддержка принята как продуктовая цель, но это отдельный порт,
а не флаг в SwiftPM: текущий код завязан на AppKit, AVAudioEngine,
Keychain, TCC/Accessibility и macOS paste/hotkey APIs.

Дорожная карта Windows-версии: [`docs/windows-support-plan.md`](docs/windows-support-plan.md).

## Что делает

- Запускается как обычное macOS-приложение с Dock-иконкой и главным
  окном. Menu bar иконка остаётся как быстрый контроллер для записи,
  смены провайдера и аварийных действий.
- Главное окно объединяет Dashboard, историю, настройки и Setup/
  Permissions. Кнопка записи из Dashboard перед стартом прячет окно и
  возвращает фокус в предыдущее приложение, чтобы auto-paste не вставил
  текст обратно в WhisperHot.
...
- Плавающий индикатор показывает elapsed time и живой RMS-уровень во
  время записи. Пять стилей: menubar-only, mini pill, classic waveform,
  floating capsule (premium, с blur-эффектом и анимированной waveform)
  и studio (широкая тёмная панель с плотной RMS-волной в стиле
  SuperWhisper). Переключается в настройках.
...
- Опциональный авто-переключатель на локальный whisper при медленной
  сети. По умолчанию выключен. Когда включён в menu bar
  («Авто-переключение на Offline при медленной сети»), при таймауте
  cloud-провайдера (10 сек по умолчанию) WhisperHot прерывает запрос
  и догоняет локальной транскрипцией.
...
3. Запусти `WhisperHot.app` — приложение подписано Developer ID и
   нотаризовано Apple, поэтому открывается без предупреждений Gatekeeper.
   (Сборки до 0.8.1 не были нотаризованы и требовали `xattr -cr
   /Applications/WhisperHot.app` — начиная с 0.8.1 это не нужно.)
...
Всё через главное окно WhisperHot. В общем sidebar доступны Dashboard,
Recording, Providers, Post-processing, Hotkey, Privacy, Updates,
History и Setup. Старое отдельное окно **Settings…** в menu bar
оставлено как fallback для быстрых действий и использует те же секции.
Интерфейс на русском и английском (переключается в Recording → Язык
интерфейса).
...
- **Recording** — язык (auto + 15 языков), auto-paste в активное
  ...
  studio), Launch at login через `SMAppService.mainApp`.
...
- **Privacy** — тумблер локальной истории транскриптов
...
- **Updates** — проверка GitHub Releases и информация о текущей версии.
- **History** — встроенный просмотр локальной истории транскриптов.
- **Setup** — состояние разрешений Microphone, Accessibility и Input
  Monitoring, готовность текущего STT-провайдера/API-ключа и кнопки
  перехода в системные настройки или Providers.
...
- Только Apple Silicon (arm64). Поддержка Intel Mac не входила в
  цели MVP.
- Windows пока не поддерживается. План порта зафиксирован в
  [`docs/windows-support-plan.md`](docs/windows-support-plan.md).
...
Смотри [`decisions.md`](decisions.md) для ADR-журнала — **почему**
сделан каждый архитектурный выбор (а не «что» и «как»).
