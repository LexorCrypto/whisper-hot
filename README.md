...
Статус: **0.6.9** — personal build. Подписан стабильным локальным
...
Studio в стиле SuperWhisper), контекстный роутинг по активному
приложению, мульти-провайдер пост-обработка, локальный LLM cleanup,
технический словарь, офлайн-фоллбек на whisper.cpp и опциональное
авто-переключение на offline при медленной сети. Sleep/wake recovery:
приложение больше не виснет после возврата Mac'а из спящего режима.
