...
приложение. 43 Swift файла, ~7550 строк. Три SwiftPM target:
...
```
Sources/WhisperHot/
├── WhisperHotApp.swift           @main entry point; регистрирует defaults
...
│   └── UpdateChecker.swift       GitHub Releases API, semver compare
│
├── Localization/
...
`~/Library/Caches/WhisperHot-build/`, не под `./build/` в корне
...
DMG как `WhisperHot-<version>.dmg`.
