...
- **Объём проверки.** Автоматически подтверждены: сборка, 56 тестов,
  платформенные предпосылки выше. Живой сценарий «подключить AirPods
  посреди записи» проверяется вручную на железе — автотест для него
  переключал бы системное устройство ввода всей машины, поэтому его нет.
