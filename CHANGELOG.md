...
- **Меню-бар больше не подвисает при сбое переключения микрофона.** Если
  обработчик аудио заклинивал в момент смены устройства, приложение до
  десяти раз пыталось дождаться его освобождения, каждый раз занимая
  главный поток на полсекунды — интерфейс замирал почти на пять секунд.
  Теперь такой отказ распознаётся сразу: запись прекращается, интерфейс
  остаётся отзывчивым. Логика обычного переключения микрофона, включая
  AirPods, этим фиксом не менялась.

### Для разработчиков

- `AudioError.tapDrainTimedOut` стал **терминальным**: перехватывается
  отдельным типизированным `catch` в `attemptDeviceSwitch` и ведёт прямо
  на give-up, минуя общий ретрай-путь. Худший случай блокировки главного
  потока на этом пути — один дренаж `tapDrainTimeout = 500 ms` вместо
  `deviceSwitchMaxAttempts × tapDrainTimeout`. Заклинивший callback не
  расклинивается повтором, поэтому ретраи здесь были чистым вредом.
  Ретраи для `invalidInputFormat` / `converterUnavailable` /
  `engineStartFailed` не изменились: устройство действительно может
  подняться со второй попытки.
- Give-up вынесен в `abandonSwitch(reason:)` — обе ветки шли одним кодом.
- Найдено Codex-аудитом диапазона `396fb9a..7e9b4e0` как `[P1]`; это же
  было открытым вопросом №20 в `SPEC.md`. Запись аудита —
  `docs/operations/audits/62a188b-codex-audit.md`.
- Нормативно закреплено: `SPEC.md` SR-AUD-019 (плюс уточнены SR-AUD-009,
  диаграмма §4.2, таксономия ошибок §8.1), `decisions.md` ADR-019 пункт 4.
- Проверено: `swift build`, `swift test` 56/56. Поведение на живом железе
  не проверялось — путь требует искусственно заклиненного tap-callback.


## [0.9.1] — 2026-07-27

Диктовка с активными AirPods снова работает. Смена аудиоустройства во время
записи теперь переносит запись на новый микрофон вместо остановки.

### Для пользователей

- **Подключение или отключение наушников не обрывает запись.** Раньше любая
  смена аудиоустройства во время диктовки останавливала её и выбрасывала
  надиктованное; после этого приложение часто отказывалось записывать до
  перезапуска. Теперь запись переезжает на новый микрофон и продолжается в
  тот же файл — теряется только доля секунды на переключение.
- **Bluetooth-микрофону даётся время подняться.** AirPods готовы не в тот
  же миг, когда macOS переключает вход: приложение ждёт до ~1.5 секунды,
  делая до 10 попыток, вместо того чтобы сдаться на первой.
- **Запись останавливается сама только в крайнем случае** — если микрофон
  так и не поднялся. Раньше это происходило при каждой смене устройства.
- **Первая запись после смены микрофона больше не падает.** Приложение
  проверяет перед стартом, не сменилось ли системное устройство ввода, и
  переподключается к актуальному.

### Для разработчиков

- `AudioRecorder`: обработчик `.AVAudioEngineConfigurationChange` вместо
  `stopRecording()` + `onAutoStop()` мигрирует сессию —
  `rebindActiveSessionToCurrentDevice()` создаёт successor-`ActiveSession`
  с новым `id` / `converter` / `inputFormat`, но **наследует** `audioFile`,
  `outputURL`, `writerQueue` и `tapGroup` (тот же дубль, одна serial-очередь
  записи). Порядок трёхфазный: сначала всё fallible, что живую сессию не
  трогает (пересборка движка, формат, конвертер); затем слот очищается и
  идёт ограниченный дренаж старого `tapGroup` — при его таймауте исходная
  сессия возвращается в слот; и только потом публикуется successor и
  стартует движок. Пока попытки продолжаются, ни один путь отказа не теряет
  дубль и не отдаёт один `AVAudioFile` двум писателям. Терминальный выход —
  исчерпание всех попыток — дубль выбрасывает: `handleAutoStop()` возвращает
  меню в `idle`, частичная запись не транскрибируется.
- Ретраи: `deviceSwitchMaxAttempts = 10`, `deviceSwitchRetryDelay = 0.15`,
  дренаж tap-callback'ов ограничен `tapDrainTimeout = 500 ms`
  (новый `AudioError.tapDrainTimedOut`). Отложенная попытка гейтится
  `deviceSwitchGeneration`, чтобы ретрай от завершённого дубля не дотянулся
  до следующего. Give-up идёт через неблокирующий `resetAfterWake()`:
  `stopRecording()` ждёт `tapGroup` на main thread и подвесил бы меню-бар,
  если callback заклинил.
- `engine` стал `var`; `rebindEngineToCurrentInputDeviceIfNeeded()`
  пересобирает `AVAudioEngine`, когда default input device сменился с
  момента сборки движка (`engineInputDeviceID`) или формат стал непригодным.
  Сравнивать `auAudioUnit.deviceID` нельзя: движок сидит на приватном
  `CADefaultDeviceAggregate-*`, а `setDeviceID(_:)` на материализованном
  юните падает с `-10851`. `resetAfterWake()` теперь тоже пересобирает
  движок, а не просто останавливает.
- `installConfigurationObserver()` сделан идемпотентным (снимает прежний
  observer), иначе переиспользование движка на switch'е протекало бы
  подписками.
- Закрыт F027 из `TECH_DEBT_AUDIT.md`: обработчик гейтится на `isRecording`,
  двойного `stopRecording()` больше нет.
- Новый `Tests/WhisperHotTests/AudioDeviceSwitchSmokeTests.swift` (read-only,
  без мутации системных настроек) фиксирует две платформенные предпосылки:
  свежий движок отдаёт пригодный и конвертируемый в 16 кГц mono формат;
  `auAudioUnit.deviceID` не совпадает с физическим input-устройством.
  Тестов стало 56. Решение целиком — `decisions.md`, **ADR-019**.
- **Объём проверки.** Автоматически подтверждены: сборка, 56 тестов,
  платформенные предпосылки выше. На живом железе владелец подтвердил один
  случай: диктовка с активными AirPods работает. Отдельно НЕ проверялись
  горячая замена устройства посреди уже идущей фразы и старт записи сразу
  после смены устройства; автотеста для них нет, потому что он переключал
  бы системное устройство ввода всей машины.


## [0.9.0] — 2026-07-21

Индикатор записи показывает, куда попадёт распознанный текст; стилей теперь два.

### Для пользователей

- **Индикатор показывает цель вставки.** Рядом с таймером записи теперь видно
  «→ приложение» — куда будет вставлен распознанный текст (активное приложение,
  зафиксированное в момент старта записи). Если авто-вставка выключена, нет
  доступа Accessibility или подходящего приложения — показывается «Буфер обмена»
  (текст всё равно копируется в буфер). Длинные имена приложений обрезаются.
- **Таймер и цель теперь во всех стилях, включая «Минимальный».** Раньше
  маленький индикатор показывал только точку и волну; теперь везде есть время
  записи mm:ss и цель вставки.
- **Стилей индикатора два: «Минимальный» и «Средний».** Стиль «Большой» удалён;
  ранее сохранённое значение «Большой» отображается как «Минимальный» (дефолт).
- **Индикатор всегда поверх всех окон во время записи.** Панель на уровне
  статус-бара и переподнимается при активации любого приложения — её больше
  не перекрывают окна, всплывшие уже после старта записи.

### Для разработчиков

- `IndicatorViewModel` получил `@Published destination`;
  `IndicatorController.show(destination:)` прокидывает его.
  `MenuBarController.pasteDestinationLabel()` формирует честный ярлык: имя
  приложения при `autoPaste && AXIsProcessTrusted() && recordingTarget`, иначе
  `L10n.indicatorClipboardTarget` («Буфер обмена» / «Clipboard»).
- `IndicatorStyle` сокращён до `minimal` / `medium`; `case large` и
  `LargeIndicatorView.swift` удалены. `MinimalIndicatorView` / `MediumIndicatorView`
  показывают таймер + «→ цель» с обрезкой (`lineLimit(1)` + `maxWidth`).
  Неизвестный сохранённый rawValue (в т.ч. `large`) → `.minimal`.
- `IndicatorController` держит панель поверх всех окон: `.statusBar` level +
  ре-ассерт `orderFrontRegardless()` по `NSWorkspace.didActivateApplication`
  (observer ставится в `show()`, снимается в `hide()`).
- Лендинг: карточка стилей индикатора обновлена (3 → 2 + цель вставки).
  README / ARCHITECTURE / CLAUDE.md синхронизированы.
- Версия поднята до 0.9.0 (CFBundleVersion 22). Developer ID + нотаризация.

## [0.8.1] — 2026-07-19

Нотаризация сборки: убрано предупреждение Gatekeeper.

### Для пользователей

- **DMG нотаризован Apple.** При скачивании и первом запуске macOS больше не
  показывает предупреждение «не проверено / переместить в Корзину» —
  приложение открывается как обычно. Снимать карантин `xattr -cr` больше не
  требуется.
- Функционально идентично 0.8.0 (иконка, три стиля индикатора, автоверсия
  сайта) — меняется только способ распространения.

### Для разработчиков

- `build-dmg.sh` запущен с `SIGNING_MODE=developer-id NOTARIZE=yes`: submit в
  Apple notary service → staple тикета → `spctl` = accepted.
- Версия 0.8.1 (CFBundleVersion 21). Настроен keychain-профиль
  `WhisperHotNotary` (app-specific password хранится локально).
