...
Откат релиза 0.6.3 целиком. Версия 0.6.3 изъята (тег и release удалены),
её изменения — расширенный fallback на локальный whisper, Studio-панель
индикатора, перегенерированные UI-звуки — откачены. Поведение совпадает
с 0.6.2. Версия бампнута до 0.6.4, чтобы авто-проверка обновлений
увидела фикс у пользователей, успевших поставить 0.6.3.
