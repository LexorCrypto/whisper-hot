...
## ADR-019 — Смена аудиоустройства во время записи: миграция сессии, а не auto-stop

**Контекст.** Пользователь: «надеваю AirPods — весь ввод перестаёт работать;
приходится снять их, выйти из приложения и запустить заново, и только тогда
работает — снова без AirPods». Что из этого **подтверждено кодом**:
обработчик `.AVAudioEngineConfigurationChange` в `AudioRecorder` делал
`stopRecording()` + `onAutoStop()`, а `MenuBarController.handleAutoStop()`
молча выбрасывает дубль записи — то есть первая половина симптома
(«запись обрывается») объясняется полностью. Вторая половина
(«не работает, пока не перезапустишь приложение») остаётся
**воспроизведённой пользователем, но не инструментально подтверждённой**:
механизм залипания мы не измеряли, потому что автоматический тест менял бы
глобальную системную настройку. Наиболее вероятная причина — движок
остаётся привязан к исчезнувшей HAL-топологии и отдаёт непригодный формат
на последующих `startRecording()`; фикс закрывает и её, и любой другой
...
не микрофона, и не годится как признак «на каком я устройстве».
`auAudioUnit.setDeviceID(_:)` на уже материализованном юните падает с
`-10851` и оставляет `deviceID = 0`, то есть прибивать движок к устройству
вручную нельзя.

**Решение.**

1. Смена конфигурации во время записи — не конец дубля, а **миграция
   сессии**: движок пересобирается, создаётся новый `AVAudioConverter` под
   новый input-формат, tap ставится заново, а `AVAudioFile`, `outputURL`,
   `writerQueue` и `tapGroup` **наследуются** — это тот же дубль, и его
   записи обязаны остаться упорядоченными на одной serial-очереди.
2. Признак «микрофон сменился» — сравнение default input device id, который
   мы **запоминаем сами** при сборке движка (`engineInputDeviceID`), а не
   опрос движка.
3. Bluetooth не готов в момент уведомления: до 10 попыток с шагом 150 мс
   (≈1.5 с) вместо отказа по первому нулевому формату.
4. **Уточнение от 2026-07-27 (0.9.2).** Из ретрай-политики пункта 3
   изъят один случай: `AudioError.tapDrainTimedOut` **терминален и не
   ретраится**. Дренаж tap-callback'ов ждёт на главном потоке, поэтому
   ретраи умножали блокировку до `deviceSwitchMaxAttempts × tapDrainTimeout`
   — до 5 секунд замороженного меню-бара, ровно тот стопор, ради
   предотвращения которого таймаут и вводился. Заклинивший callback не
   расклинивается повтором. Найдено Codex-аудитом диапазона
   `396fb9a..7e9b4e0` как `[P1]`; в первой редакции решения это было
   ошибкой. Нормативно закреплено в `SPEC.md` SR-AUD-019.
...
прибить движок к устройству API не даёт (см. `-10851` выше), спросить у
движка его микрофон нельзя (агрегат), а свежий движок заведомо отдаёт
пригодный формат — это единственное, что здесь проверено тестом
(`AudioDeviceSwitchSmokeTests`). Наследование
`writerQueue`/`tapGroup` нарушает изоляцию из ADR-015 ровно в одном
контролируемом месте; альтернатива — новая очередь поверх того же
`AVAudioFile` — это гонка на записи в один файл. `stopRecording()` на
give-up-пути запрещён: один из режимов отказа — заклинивший tap-callback, а
`stopRecording()` ждёт `tapGroup` на main thread и подвесил бы меню-бар;
`handleAutoStop()` всё равно выбрасывает дубль, так что чистый flush WAV
там ничего не даёт.

**Последствия.**

- **Подтверждено на живом железе 2026-07-27** (владелец, установленная 0.9.1),
  дословно: диктовка с активными AirPods работает — приложение слышит
  голос через наушники. Это ровно тот случай, который был исходным
  запросом («сделать, чтобы при активных AirPods тоже работало»).
- **Не проверялось отдельно** и потому не считается закрытым: (а) горячая
  замена — надеть или снять AirPods ровно посреди уже идущей фразы;
  (б) прежнее залипание — что после смены устройства последующие записи
  стартуют без перезапуска приложения. Механизм для обоих один и тот же
  (`handleConfigurationChange` → миграция сессии, сверка default input
  перед стартом), но ручного прогона именно на них не было.
- Закрыт F027 из `TECH_DEBT_AUDIT.md`: обработчик теперь гейтится на
  `isRecording`, двойной `stopRecording()` невозможен.
- `AudioRecorder.engine` перестал быть `let`. Появился `AudioError.tapDrainTimedOut`.
- Полный сценарий «переключить устройство на живой записи» остаётся ручной
  проверкой: автоматический тест менял бы глобальную системную настройку.
  В `Tests/WhisperHotTests/AudioDeviceSwitchSmokeTests.swift` зафиксированы
  только read-only предпосылки (свежий движок отдаёт пригодный формат;
  `auAudioUnit.deviceID` не равен физическому input).

**Альтернативы.**

- *Оставить auto-stop и просто чинить залипание.* Отвергнуто: пользователь
  всё равно теряет фразу при каждом подключении наушников.
- *Прибить движок к устройству через `setDeviceID`.* Отвергнуто
  экспериментально: `-10851` и порча юнита.
- *Сравнивать `auAudioUnit.deviceID` с default input.* Отвергнуто: движок
  сидит на агрегате, сравнение всегда ложно-положительное.
- *Заводить новую `writerQueue` на успешника.* Отвергнуто: две очереди на
  один `AVAudioFile` — гонка.
