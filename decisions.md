...
- Ожидаемый эффект: надел или снял AirPods посреди фразы — запись
  продолжается, теряется доля секунды на переключение. Подтверждение —
  ручной прогон на живом железе (см. ниже), не автотест.
- Залипание должно уйти: `startRecording()` пересобирает движок, если
  default input сменился с момента прошлой сборки или формат стал
  непригодным. Ожидание, а не измерение — см. оговорку в «Контексте».
