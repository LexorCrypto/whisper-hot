...
HEAD origin/main` совпадает с `HEAD`, `audit_coverage.py` показывает `session 0`.
Ручную audit-запись такие коммиты получить всё ещё могут — что и было сделано в
этой сессии, — но **гейт их больше не считает**: он не подтвердит покрытие и не
заметит, если записи не будет вовсе. Так вышло с 0.9.1 и 0.9.2: четыре раунда
пришлось запускать руками вне гейта. Работа идёт в
