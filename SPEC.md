...
Каждое нормативное требование сопровождается ссылкой одного из трёх видов: `путь/файл.swift:строки` (точный код на 0.9.2), `ADR-NNN` (запись в `decisions.md`), либо `PRD §N.M` / `AC-N.k` (раздел PRD). Пункты, для которых источник истины в репозитории не найден, помечены **[УТОЧНИТЬ]** с указанием, где заведено обсуждение (см. раздел 14). Пункты, выведенные логически из нескольких источников, а не процитированные дословно, помечены **[ИНФЕРЕНЦИЯ]**.
...
- Захват голоса, конвейер транскрипции (5 провайдеров STT), опциональная LLM-постобработка (6 провайдеров), контекстный роутинг, технический словарь, вставка текста, история, индикатор записи, все экраны Settings/Onboarding/Setup, локализация (ru/en) — весь код в `Sources/WhisperHot/` (45 Swift-файлов, 9294 строки на момент написания; точное число дрейфует почти с каждым коммитом — пересчитывайте `find Sources/WhisperHot -name '*.swift' | xargs wc -l`, см. `ARCHITECTURE.md:3-6`).
...
- **App Sandbox** — не активирован и не планируется без пересмотра архитектуры local-режимов: офлайн-провайдеры запускают внешние исполняемые файлы через `Process`, что несовместимо с sandbox (`Resources/WhisperHot.entitlements:1-8`; `decisions.md:169-182,552`).
...
- **Публичный контракт.** `enum L10n` — static computed properties/functions по разделам UI (`L10n.swift:20-375`); `enum AppLanguage` — 2 кейса (ru, en, `L10n.swift:4-16`).
...
| `migrating` | `rebindActiveSessionToCurrentDevice()` успех | активная сессия #N+1 (successor, наследует `audioFile`/`outputURL`/`writerQueue`/`tapGroup`, новый `id`/`converter`/`inputFormat`) | новый tap, `installConfigurationObserver()`, `engine.start()` | `AudioRecorder.swift:473-525`; ADR-019 |
| `migrating` | рестарт бросает, `attempt+1 < 10` | `migrating` (то же generation) | `DispatchQueue.main.asyncAfter(+150ms)` → повторная попытка | `AudioRecorder.swift:433-441` |
| `migrating` | рестарт бросает, `attempt+1 == 10` (give-up) | нет сессии | `resetAfterWake()` (неблокирующий) + `onAutoStop?()` — WAV не транскрибируется | `AudioRecorder.swift:434-436,446-452` |
...
| `migrating` | любой новый lifecycle-переход (start/stop/reset) до resolve | (generation инвалидировано) | `invalidatePendingDeviceSwitch()`; отложенный ретрай со старым generation молча дропается | `AudioRecorder.swift:417,530-533` |
...
stateDiagram-v2
    ...
    Migrating --> NoSession: rebind fails, attempt == 10\nИЛИ tapDrainTimedOut (терминален, без ретрая)\n(resetAfterWake + onAutoStop,\nnot transcribed, WAV orphaned)
    Recording --> NoSession: willSleep / resetAfterWake()\n(WAV orphaned on disk, retention sweeper cleans up)
    Migrating --> NoSession: stopRecording() between retries\n(isRecording still true, WAV returned normally)
    Migrating --> NoSession: willSleep / resetAfterWake() between retries\n(pending retry invalidated, WAV orphaned)
```

**Ключевой инвариант ADR-019 (порядок операций).** Миграция трёхфазна, и живая сессия трогается **раньше** последнего fallible-шага — это надо читать буквально, потому что более короткая формулировка «всё fallible выполняется до изменения живой сессии» **неверна**. Фаза A (сессия нетронута): пересборка движка, чтение формата, создание конвертера — отказ здесь оставляет исходную сессию как есть. Фаза B: `sessionLock` **очищается**, затем идёт ограниченный `tapGroup.wait(timeout: tapDrainTimeout=500мс)`; при таймауте исходная сессия **возвращается** в слот и бросается `AudioError.tapDrainTimedOut` (`AudioRecorder.swift:493-497`). Фаза C: публикуется successor, ставится tap, вызывается `engine.start()`; при его отказе в слоте намеренно остаётся **successor** — он владеет тем же `audioFile` / `outputURL` / `writerQueue` / `tapGroup`, и ретрай продолжает тот же дубль. Инвариант, который действительно держится на ретрай-путях: дубль не теряется, и один `AVAudioFile` никогда не отдаётся двум писателям одновременно. Терминальный выход — исчерпание попыток (SR-AUD-012) либо `tapDrainTimedOut` (SR-AUD-019) — ведёт по цепочке `abandonSwitch` → `resetAfterWake()` → `onAutoStop()`; частичная запись **не транскрибируется** (SR-AUD-013, §8.1). WAV при этом **не удаляется**: `resetAfterWake()` лишь логирует «orphaned WAV» (`AudioRecorder.swift:360-362`) — файл остаётся на диске и уходит только согласно выбранной политике `AudioRetention` (§5.6): `.forever` не удаляет его автоматически никогда, `.untilQuit` стирает при выходе из приложения и компенсаторно на следующем запуске, прочие политики — по возрастному sweep.
...
| Расширение файла | `.wav` | `AudioRecorder.swift:623` |
| Путь | `~/Library/Caches/WhisperHot/recordings/<UUID>.wav` (`.cachesDirectory` + `WhisperHot/recordings` + `UUID().uuidString + ".wav"`) | `AudioRecorder.swift:613-625` |
| Размер буфера захвата | 4096 фреймов на tap-callback | `AudioRecorder.swift:192,511` |
...
| OpenRouter STT | `POST https://openrouter.ai/api/v1/chat/completions`, заголовки `HTTP-Referer: https://github.com/aleksejsupilin/whisper-local`, `X-Title: WhisperHot` | JSON: `model`, `modalities:["text"]`, `temperature:0`, системный промпт «вернуть только транскрипт», `input_audio`(base64 WAV) | Аудио (base64), язык/подсказка в тексте промпта | 120с / 180с | 2xx → `choices[0].message.content`; пусто → `emptyTranscript` |
...
Всего **135** требований в 12 доменах. Каждое заземлено ссылкой на код (0.9.2), ADR или раздел PRD.
...
| SR-AUD-009 | При неудачной попытке миграции приложение **ОБЯЗАНО** повторить попытку до `deviceSwitchMaxAttempts=10` раз; первая попытка (attempt 0) выполняется синхронно, задержка `deviceSwitchRetryDelay=0.15` с ставится через `asyncAfter` только перед попытками 1…9 — девять задержек, **1.35 с фиксированной суммы** плюс время самих 10 попыток, а не «≈1.5с». Исключение — `AudioError.tapDrainTimedOut`, который **НЕ ДОЛЖЕН** ретраиться (SR-AUD-019). | `AudioRecorder.swift:80-81,413-442`; `decisions.md:485-486` (ADR-019) | §3.2, AC-2.2 |
| SR-AUD-010 | Ожидание дренажа tap-callback'ов старой сессии при миграции **ОГРАНИЧЕНО** таймаутом `tapDrainTimeout=500` мс; при превышении **ОБЯЗАНА** быть брошена `AudioError.tapDrainTimedOut`, а исходная сессия — возвращена в `sessionLock` без изменений. | `AudioRecorder.swift:86,493-497` | §3.2 |
| SR-AUD-019 | `AudioError.tapDrainTimedOut` **ОБЯЗАН** быть терминальным: перехватываться отдельным типизированным `catch` и вести прямо на `abandonSwitch(reason:)`, **НЕ ДОЛЖЕН** попадать в общий ретрай-путь. Обоснование: дренаж ждёт на главном потоке, поэтому ретраи умножали бы блокировку до `deviceSwitchMaxAttempts × tapDrainTimeout` (до 5 с замороженного меню-бара) — ровно тот стопор, ради предотвращения которого таймаут и введён. Заклинивший callback не расклинивается повтором. Худший случай блокировки главного потока на этом пути — **один** дренаж 500 мс. | `AudioRecorder.swift:427-432,444-452`; `decisions.md:487-495`; находка [P1] Codex-аудита `396fb9a..7e9b4e0` | §3.2 |
| SR-AUD-011 | Каждая попытка миграции **ОБЯЗАНА** быть привязана к поколению (`deviceSwitchGeneration`); отложенный ретрай с несовпадающим поколением **ОБЯЗАН** быть отброшен без действия (`guard generation == deviceSwitchGeneration else { return }`). Поколение инкрементируется двумя разными путями: начало НОВОЙ миграции — инлайн-инкремент в `handleConfigurationChange()`; отмена ОЖИДАЮЩЕГО ретрая обычным lifecycle-переходом (`startRecording()`/`stopRecording()`/`resetAfterWake()`) — через `invalidatePendingDeviceSwitch()`. Во время активной миграции `isRecording` остаётся `true`, поэтому вызов `startRecording()` в этот момент **ОБЯЗАН** сразу вернуть `AudioError.alreadyRecording` на guard'е `!isRecording`, не доходя до инвалидатора. | `AudioRecorder.swift:74,402-403,413-417,530-533` | §3.2 |
| SR-AUD-012 | Если ни одна из `deviceSwitchMaxAttempts` попыток не дала пригодного формата, приложение **ОБЯЗАНО** остановить запись через неблокирующий `resetAfterWake()`, а **НЕ ДОЛЖНО** вызывать `stopRecording()` на этом пути (блокирует главный поток на `tapGroup.wait()` при заклинившем callback). | `AudioRecorder.swift:434-436,446-452`; `decisions.md:508-512` (ADR-019) | §3.2, AC-2.3 |
...
| SR-AUD-018 | `computeRMS()` **ОБЯЗАН** возвращать `0` для любого формата буфера, отличного от `.pcmFormatFloat32` — известное ограничение; отдельное диагностическое логирование первого такого несовпадения **НЕ РЕАЛИЗОВАНО** на 0.9.2 (см. раздел 14). | `AudioRecorder.swift:627-631`; `TECH_DEBT_AUDIT.md:52` (F017) | §4 (НФТ) |
...
| SR-HK-005 | Fn-транспорт и Carbon-транспорт **ВЗАИМОИСКЛЮЧАЮЩИ**: при включении `Preferences.fnKeyEnabled` приложение **ОБЯЗАНО** сначала попытаться запустить `FnKeyMonitor` и снять регистрацию Carbon (`hotkeyManager.unregister()`) **только** при успехе; при неудаче приложение **ОБЯЗАНО** попытаться (заново) зарегистрировать Carbon через `hotkeyManager.register(combo:)`. Гарантии, что Carbon в итоге останется зарегистрированным, код не даёт: `register()` безусловно вызывает `unregister()` первой строкой, и если сам `RegisterEventHotKey` в этой попытке тоже откажет, приложение временно останется без работающего хоткея (залогировано как «failed to register Carbon fallback hotkey»). | `MenuBarController.swift:300-328`; `HotkeyManager.swift:57-107` | §3.12 |
...
| SR-STT-011 | Непустая (после trim) подсказка распознавания (`Preferences.vocabularyHints`) **ОБЯЗАНА** передаваться выбранному STT-провайдеру одним из трёх разных способов в зависимости от транспорта: OpenAI-совместимые провайдеры (OpenAI/Groq/Polza.ai) — multipart-полем `prompt`; OpenRouter — текстом внутри `user`-сообщения («Context hints: …», НЕ отдельным API-полем); Local whisper.cpp — флагом `--prompt` subprocess'а. | `MenuBarController.swift:918-921`; `OpenAICompatibleSTTProvider.swift:86-88`; `OpenRouterAudioProvider.swift:76-78`; `LocalWhisperProvider.swift:50-52` | §3.7, AC-7.3 |
...
| SR-CTX-004 | Правило с непустым `titleContains` **ОБЯЗАНО** читать заголовок фокусного окна лениво: `AXUIElementCopyAttributeValue` вызывается только когда встречено правило, требующее title, без предварительной проверки разрешения Accessibility. При любом неуспехе AX-запроса (в т.ч. из-за отсутствия разрешения) правило **ОБЯЗАНО** быть молча пропущено — `windowTitle(for:)` просто возвращает `nil`. | `ContextRouter.swift:39-48,58-65,75-92` | §3.6, AC-6.3 |
...
| SR-SEC-004 | `Keychain.readAPIKey`/`readData` **ОБЯЗАНЫ** транслировать `errSecItemNotFound` в `KeychainError.itemNotFound`, а любой другой ненулевой `OSStatus` — в `KeychainError.unexpectedStatus`; UI **ОБЯЗАН** показать пользователю численный `OSStatus` этой ошибки (например «Keychain error (OSStatus -34018).» — код подставляет только число, не текст системной ошибки), а **НЕ** тихо считать операцию успешной. | `Keychain.swift:19-31` | §3.11, AC-11.4 |
...
| SR-SEC-012 | Единственный конвейерный сетевой трафик диктовки **ОБЯЗАН** уходить только по HTTPS выбранному пользователем провайдеру; обновление приложения, установка локальной модели с HuggingFace и запуск Homebrew (`brew install whisper-cpp`, с полным унаследованным окружением процесса) — единственные прочие исходящие действия, и они **НЕ ДОЛЖНЫ** содержать содержимое диктовки. | `UpdateChecker.swift:43`; `WhisperInstaller.swift:26,144-145` | §3.15 |
...
| ID | Требование | Заземление | PRD |
|---|---|---|---|
| SR-UI-001 | Индикатор **ОБЯЗАН** поддерживать ровно два визуальных стиля — Minimal и Medium; неизвестное/устаревшее сохранённое значение (включая удалённый в 0.9.0 стиль Large) **ОБЯЗАНО** откатываться к `.minimal`. | `Preferences.swift:451-456`; `CHANGELOG.md:123-124` | §3.9, AC-9.4 |
...
| SR-UI-007 | Интерфейс **ОБЯЗАН** поддерживать ровно два языка — русский (по умолчанию) и английский. В Settings переключение **ОБЯЗАНО** применяться немедленно к элементам, связанным через `@AppStorage(Preferences.Key.appLanguage)` (например к самому переключателю языка). **[ИНФЕРЕНЦИЯ]** Для прочих SwiftUI-поверхностей (Main Window/Setup), которые читают `L10n.*` как обычные статические вычисляемые свойства без собственной привязки к `appLanguage`, немедленное обновление при переключении языка в другом окне не подтверждено — `MainWindowModel` не подписан на изменение `appLanguage` и пересчитывает `body` по другим триггерам (`refresh()`, охватывающему только permission/setup-состояние). | `L10n.swift:3-24`; `Preferences.swift:77`; `SettingsView.swift:81,188-192`; `MainWindow/MainWindowController.swift:92-158,280-282` | §3.13, AC-13.1 |
...
| SR-UI-010 | Окно Onboarding и окно истории остаются захардкожены на английском независимо от выбранного языка — задокументированный известный пробел покрытия локализации на 0.9.2, а не нормативный дефект. | `OnboardingWindowController.swift:45-217`; `TranscriptHistoryView.swift:25-140` | §3.13, AC-13.2 |
...
| SR-UI-012 | Sticky-баннер ошибки **ОБЯЗАН** очищаться при старте новой записи и при успешной транскрипции без ошибки постобработки; клик по баннеру (`postProcessingErrorTapped`) **ОБЯЗАН** открывать Settings и **НЕ ДОЛЖЕН** сам по себе очищать баннер. Заголовок **ОБЯЗАН** обрезаться до 80 символов, полный текст — доступен через tooltip. | `MenuBarController.swift:799,1028-1044` | §3.5, AC-5.4 |
...
| SR-PERM-003 | Input Monitoring запрашивается двумя независимыми путями: (1) при включении тумблера экспериментального Fn-хоткея в Settings (`onChange` вызывает `requestInputMonitoring()`); (2) кнопкой в постоянно видимой строке Setup (`MainWindowController.openInputMonitoringSettings()`), которая безусловно вызывает `requestInputMonitoring()` независимо от состояния `fnKeyEnabled`. В обоих случаях отказ или отсутствие этого разрешения **НЕ ДОЛЖНЫ** ломать штатный Carbon-хоткей. | `PermissionsCoordinator.swift:53-66`; `FnKeyMonitor.swift:8-16`; `SettingsView.swift:705-712`; `MainWindow/MainWindowController.swift:193-197,557-564` | §3.12, AC-12.4 |
...
| SR-PERM-005 | Закрытие окна Onboarding **ОБЯЗАНО** безусловно устанавливать `WhisperHot.hasShownOnboarding=true`, независимо от того, все ли разрешения выданы. | `OnboardingWindowController.swift:232-246` | §3.12 |
...
| SR-REL-002 | `build.sh` **ОБЯЗАН** по умолчанию собирать release-конфигурацию со стабильной локальной подписью (`SIGNING_MODE=local`, сертификат `whisper-hot-local`); режим `developer-id` **ОБЯЗАН** требовать непустую Developer ID Application identity, заданную через `DEVELOPER_ID_APPLICATION_IDENTITY` (приоритетно) либо, как fallback, через `SIGNING_COMMON_NAME` — обе переменные окружения принимаются. | `build.sh:7-8,19-40` | §3.14 |
...
| SR-REL-009 | Начиная с релиза `0.6.6`, `CHANGELOG.md` **ОБЯЗАН** получать новую секцию `## [<версия>] — <дата>` с подразделами «Для пользователей» и «Для разработчиков» при каждом релизе, меняющем видимое поведение; это правило действует для текущих и будущих релизов. Исторические записи `0.6.4` и `0.6.5` содержат только подраздел «Для пользователей» и под правило не подпадают. | `CHANGELOG.md:1-9,633-651,683-708` (формат подтверждён без перерыва от `0.6.6` до текущей версии) | §3.14 |
...
| SR-PERF-005 | Захват аудио с input-тапа **ОБЯЗАН** использовать буфер размером 4096 фреймов как на первичном, так и на любом successor-тапе после миграции устройства. | `AudioRecorder.swift:192,511` | §3.2 |
...
| `engineStartFailed(underlying:)` (initial start) | `engine.start()` бросил ошибку при `startRecording()` | Не показывается отдельно — лог | Повторная попытка старта записи | Нет — только что созданный WAV реально удаляется (`removeItem`, файл уже существовал) |
| `engineStartFailed(underlying:)` (migration recovery) | `engine.start()` бросил ошибку в Фазе C `rebindActiveSessionToCurrentDevice()` | Не показывается пользователю напрямую | Ретрай — successor **намеренно остаётся** в `sessionLock`, владея тем же `audioFile`/`outputURL` | Нет — файл НЕ удаляется, тот же дубль продолжается на следующей попытке |
| `tapDrainTimedOut` (migration recovery, Фаза B) | Дренаж `tapGroup` при миграции устройства превысил 500 мс | Не показывается пользователю напрямую | **Терминален, не ретраится** — сразу auto-stop через неблокирующий `resetAfterWake()` (SR-AUD-019) | WAV **не удаляется** — остаётся orphaned на диске, транскрипция не запускается; удаляется только согласно политике `AudioRetention` (§5.6) |

**Примечание про удаление файла.** В `startRecording()` реальное удаление (`try? FileManager.default.removeItem`) — no-op для `invalidInputFormat`/`converterUnavailable`, поскольку `AVAudioFile` там ещё не создан; оно становится значимым только при отказе создания самого `AVAudioFile` (ошибка прокидывается как есть, не оборачивается ни в один из этих 7 кейсов `AudioError`, `AudioRecorder.swift:149-168`) либо при отказе `engine.start()` (`AudioRecorder.swift:198-207`).
...
| `unexpectedStatus(OSStatus)` | Любой другой ненулевой Security-статус | «Save/Delete/Load failed: …» с численным `OSStatus` (например «Save failed: Keychain error (OSStatus -34018).» — не текст системной ошибки) | Диагностика Keychain (например legacy ACL, SR-SEC-014) |
...
Все значения — реальные константы кода на 0.9.2, не оценки.

| Категория | Величина | Значение | Заземление |
|---|---|---|---|
| Захват аудио | Размер tap-буфера | 4096 фреймов | `AudioRecorder.swift:192,511` |
...
| Миграция устройства | Интервал ретрая | 150 мс между попытками; 9 задержек (попытки 1…9), первая попытка синхронна — 1.35 с фиксированной суммы | `AudioRecorder.swift:80` |
...
- `CHANGELOG.md` **ОБЯЗАН** получать секцию `## [<версия>] — <дата>` с подразделами «Для пользователей»/«Для разработчиков» при каждом релизе, меняющем видимое поведение (SR-REL-009) — правило действует начиная с `0.6.6`. Под него не подпадают только записи `0.6.4` и `0.6.5`: они действительно несут единственный подраздел «Для пользователей». Версии до `0.4.0` (`0.1.0`, `0.2.1`, `0.2.2`, `0.3.0`) парный раздел для разработчиков имеют — он озаглавлен по-старому, «Для контрибуторов» (`CHANGELOG.md:924,971,1013,1154`); отсутствует у них именно заголовок «Для разработчиков», а не сам раздел.
...
### 12.1 Автотесты (56 тестов, 6 файлов — точное число получено `grep -c 'func test' Tests/WhisperHotTests/*.swift`, совпадает с заявленным в `CHANGELOG.md:37` числом для 0.9.2)

| Файл | Тестов | Что защищает |
|---|---|---|
| `ContextRouterTests.swift` | 15 | Дефолтные mapping'и (Slack/Mail/VS Code/Xcode/Telegram), generic-браузеры, unknown app, пустой список правил, custom override/fallback, порядок первого совпадения, wildcard-shadowing. **Не покрывает**: производственный `ContextRouter.resolve` вызывается только в 3 из 15 тестов (`testNilTargetMatchesFallback`, `testSlackMatchesCasual`, `testNilTargetMatchesWildcard`), все три раза с `target: nil`, причём в `testSlackMatchesCasual` результат отбрасывается через `_ =` и не утверждается ничего — это смоук-вызов, а не проверка; 12 тестов используют локальный helper `resolveByBundleID`, который переимплементирует matching-логику и явно пропускает title-based правила, а не вызывает `resolve`; реальный `NSRunningApplication` + AX-запрос заголовка окна (`windowTitle(for:)`) не тестируется вовсе — title-правило проверяется только на уровне модели `ContextRule` |
...
- **Живое переключение аудиоустройства во время записи (ADR-019, SR-AUD-004–SR-AUD-013).** Автотест изменял бы глобальную системную настройку входного устройства машины, на которой выполняется CI/разработка — недопустимо для юнит-теста. Измерено вручную на трёх устройствах (AirPods Pro, встроенный микрофон MacBook Pro, микрофон iPhone) разовым CoreAudio-прогоном (`decisions.md:464-466`); read-only платформенные предпосылки этого прогона воспроизводятся тестами в `AudioDeviceSwitchSmokeTests`, но сам живой сценарий переключения устройства — нет (`decisions.md:529-533`). `AudioDeviceSwitchSmokeTests.swift` фиксирует только эти предпосылки, НЕ сам сценарий переключения.
...
- **HTTP-контракт облачных провайдеров (SR-STT-003–SR-STT-007, SR-LLM-011).** Ни один из 6 текущих тестовых файлов не использует `URLProtocol`-мок против `OpenAICompatibleSTTProvider`/`OpenRouterAudioProvider`/`LLMPostProcessor` — multipart-тело, коды ошибок, размерные лимиты не покрыты автотестами (`TECH_DEBT_AUDIT.md:55` F020, статус не изменился к 0.9.2).
...
- **Сборка и релиз** (`build.sh`, `build-dmg.sh`, нотаризация) — не входят в `swift test`; проверяются вручную по чеклисту (`spctl -a -t open`, `xcrun stapler validate`, задокументировано в `CHANGELOG.md:167-170` для 0.8.1).

### 12.3 Критические пути без покрытия (ссылка на `TECH_DEBT_AUDIT.md`)

Аудит датирован 2026-05-08, версия 0.6.7 — часть находок закрыта к 0.9.1 (см. `TECH_DEBT_AUDIT.md` раздел «Quick wins», подтверждено кросс-проверкой ScoutOps §4), но следующие тестовые пробелы **подтверждены прямым чтением текущего `Tests/WhisperHotTests/`** и остаются актуальными на 0.9.2:

| ID аудита | Пробел | Актуален на 0.9.2? |
|---|---|---|
...
| §3.2 Устройство ввода и устойчивость к смене (ADR-019) | SR-AUD-004–016,018,019; SR-PERF-005 | AC-2.1–AC-2.4 | Частично: `AudioDeviceSwitchSmokeTests.swift` (2 теста) — только платформенные предпосылки; сам живой сценарий переключения — обязательно вручную на реальном железе (AirPods Pro / MacBook Pro mic / iPhone mic), см. §12.2 |
| §3.3 Транскрипция и провайдеры | SR-HK-002; SR-STT-001–011,015,016; SR-PERF-001,002,006 | AC-3.1–AC-3.5 | Не покрыто автотестами (F020); проверено вручную/производственным опытом релизов |
| §3.4 Офлайн-режим и fallback | SR-LLM-007; SR-PERF-004; SR-STT-008,009 | AC-4.1–AC-4.5 | Частично: `FallbackTranscriptionServiceTests.swift` (6 тестов) покрывает race-логику `FallbackTranscriptionService` на моках (`DelayedMockTranscriptionService`/`FailingMockTranscriptionService`) — timeout race, non-offline ошибка на `TranscriptionError.invalidResponse`, guard'ы тумблера; тесты **не создают** `LocalWhisperProvider`, **не вызывают** `TranscriptionCoordinator` и **не подают** `URLError.notConnectedToInternet` — реальный local-provider путь (SR-STT-008,009) не упражняется |
...
| §3.6 Контекстный роутинг | SR-CTX-001–007; SR-LLM-012 | AC-6.1–AC-6.4 | Частично: `ContextRouterTests.swift` (15 тестов) — но производственный `ContextRouter.resolve` вызывается только в 3 из 15, все три раза с `target: nil`, и один из трёх (`testSlackMatchesCasual`) отбрасывает результат через `_ =`; 12 тестов идут через локальный helper `resolveByBundleID`, переимплементирующий matching-логику; title-based AX-запрос (SR-CTX-004) не упражняется вообще, только модельные поля `ContextRule` (см. §12.1) |
| §3.7 Технический словарь и замены слов | SR-STT-011–014 | AC-7.1–AC-7.4 | Частично: `WordReplacementTests.swift` (12 тестов) покрывает `WordReplacement.apply`/`.applyAll`/`.defaults` напрямую на литеральных значениях — ни один тест не читает `Preferences.wordReplacements` и не проверяет JSON-декодирование/fallback на 16 встроенных правил (SR-STT-014); доставка `vocabularyHints` как STT-параметра (SR-STT-011) — не покрыта отдельным тестом |
...
| §3.10 История транскриптов | SR-HIST-001–009; SR-SEC-006 | AC-10.1–AC-10.5 | Частично: `HistoryStoreTests.swift` (10 тестов) + `KeychainTests.swift` (11 тестов, слой ключа шифрования) покрывают round-trip/orphan/prune/clear на уровне `HistoryStore`; ни один тест не исполняет gate `Preferences.historyEnabled` (он живёт в `MenuBarController`, вне тестового таргета), и `testClearRemovesFileAndRecords` не проверяет, что Keychain-ключ шифрования переживает `clear()` (SR-HIST-007) |
...
| §3.14 Установка, подпись и обновления | SR-REL-001–011; SR-PERF-009,010 | AC-14.1–AC-14.5 | Не покрыто `swift test`; проверено вручную по релизному чеклисту (`spctl -a -t open`, `xcrun stapler validate` — задокументировано в `CHANGELOG.md:167-170` для 0.8.1) |
...
| 7 | Нужен ли явный UI-сигнал пользователю, когда auto-stop срабатывает после исчерпания 10 попыток миграции устройства (SR-AUD-012) | `PRD.md §3.2`; `decisions.md:496-499` | UX-решение; сейчас поведение молчаливое (только `NSLog`) |
...
| 18 | Появилась ли готовность у мейнтейнера к «minimum coverage gate» в CI (аудит оценивал критично низкое покрытие на 0.6.7; на 0.9.2 покрытие выросло до 56 тестов, но провайдеры/paste/UI/ретенция всё ещё не покрыты — §12.3) | `TECH_DEBT_AUDIT.md` OQ5 | Решение мейнтейнера; не техническая блокировка |
