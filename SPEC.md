...
- `CHANGELOG.md` **ОБЯЗАН** получать секцию `## [<версия>] — <дата>` с подразделами «Для пользователей»/«Для разработчиков» при каждом релизе, меняющем видимое поведение (SR-REL-009) — правило действует начиная с `0.6.6`; записи `0.6.4` и `0.6.5`, а также ряд версий до `0.4.0`, содержат только подраздел «Для пользователей» и под правило не подпадают.
