...
**Ключевой инвариант ADR-019 (порядок операций).** Всё, что способно завершиться ошибкой при миграции — пересборка движка, чтение формата, создание конвертера, дренаж старого `tapGroup` (с таймаутом `tapDrainTimeout=500мс`) — выполняется **до** того, как живая сессия каким-либо образом изменена, **кроме одного шага**: `engine.start()` вызывается уже **после** публикации successor в `sessionLock`. Практическое следствие двух режимов отказа: (а) отказ до публикации оставляет исходную сессию нетронутой, следующий ретрай начинает с неё; (б) отказ `engine.start()` оставляет в слоте **successor**, а не исходную сессию — это сделано намеренно, потому что successor владеет тем же `audioFile` / `outputURL` / `writerQueue` / `tapGroup`, и ретрай продолжает тот же дубль ровно с этой точки. Инвариант ограничен **ретрай-путём**: пока попытки продолжаются, дубль не теряется ни в одном из режимов. Терминальный выход — исчерпание попыток (SR-AUD-012) либо `tapDrainTimedOut` (SR-AUD-019) — дубль **выбрасывает**: `abandonSwitch` → `resetAfterWake()` → `onAutoStop()`, и частичная запись не транскрибируется (SR-AUD-013, §8.1) (`AudioRecorder.swift:465-517`).
...
| SR-AUD-008 | Операции миграции, способные завершиться ошибкой **до** публикации successor (пересборка движка, получение формата, создание конвертера, дренаж старого `tapGroup`), **ОБЯЗАНЫ** выполняться до того, как живая сессия изменена; такая неудачная попытка **ОБЯЗАНА** оставить исходную сессию нетронутой. Единственное исключение — `engine.start()`, вызываемый уже после публикации: при его отказе в `sessionLock` **ОБЯЗАН** остаться successor (он владеет тем же `audioFile`/`outputURL`/`writerQueue`/`tapGroup`), чтобы ретрай продолжил тот же дубль. Требование ограничено ретрай-путём и **НЕ** распространяется на терминальный выход (SR-AUD-012, SR-AUD-019), который дубль осознанно выбрасывает. | `AudioRecorder.swift:465-517` | §3.2 |
