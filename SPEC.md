...
stateDiagram-v2
    ...
    Migrating --> Migrating: rebind fails, attempt < 10 (retry in 150ms)
...
**Ключевой инвариант ADR-019 (порядок операций).** Всё, что способно завершиться ошибкой при миграции — пересборка движка, чтение формата, создание конвертера, дренаж старого `tapGroup` (с таймаутом `tapDrainTimeout=500мс`) — выполняется **до** того, как живая сессия каким-либо образом изменена. Успешная сборка successor-сессии — единственная точка, где `sessionLock` заменяется; неудачная попытка оставляет исходную сессию нетронутой для следующего ретрая (`AudioRecorder.swift:441-451,452-504`; `decisions.md:491-503`, ADR-019 «Обоснование»).
...
Всего **134** требования в 12 доменах. Каждое заземлено ссылкой на код (0.9.1), ADR или раздел PRD.

### 7.1 `SR-AUD-*` — Аудио и устройства ввода (18)
...
| SR-AUD-008 | Все операции, способные завершиться ошибкой при миграции (пересборка движка, получение формата, создание конвертера, дренаж старого `tapGroup`), **ОБЯЗАНЫ** выполняться до того, как живая сессия каким-либо образом изменена; неудачная попытка **ОБЯЗАНА** оставить исходную сессию нетронутой. | `AudioRecorder.swift:441-451,452-504`; `decisions.md:491-503` (ADR-019) | §3.2 |
| SR-AUD-009 | При неудачной попытке миграции приложение **ОБЯЗАНО** повторить попытку до `deviceSwitchMaxAttempts=10` раз с интервалом `deviceSwitchRetryDelay=0.15` с (≈1.5с суммарно), прежде чем считать переключение неудачным. | `AudioRecorder.swift:80-81,413-439`; `decisions.md:485-486` (ADR-019); `CHANGELOG.md:17-19,35-36` | §3.2, AC-2.2 |
...
| `tapDrainTimedOut` | Дренаж `tapGroup` при миграции устройства превысил 500 мс | Не показывается пользователю напрямую — приводит к очередному ретраю миграции (SR-AUD-010) | Ретрай до 10 раз; при исчерпании — auto-stop (см. SR-AUD-012) | Частично — partial recording при полном исчерпании ретраев НЕ транскрибируется |
...
| 20 | **[ИНФЕРЕНЦИЯ, найдено при написании SPEC]** Ретрай миграции устройства после `AudioError.tapDrainTimedOut` (SR-AUD-010) использует тот же счётчик попыток (`deviceSwitchMaxAttempts`), что и ретрай после иных ошибок (`invalidInputFormat`/`converterUnavailable`/`engineStartFailed`) — код не различает «заклинивший callback, вряд ли разрешится за 1.5с» от «временно неготовый формат устройства, вероятно разрешится». Требует ли `tapDrainTimedOut` отдельной, более короткой политики ретраев (или немедленного give-up) — не описано ни в ADR-019, ни в коде явно | `AudioRecorder.swift:413-439,472-476` | Технический вопрос; владелец — автор `AudioRecorder`/ADR-019 |
| 21 | **[ИНФЕРЕНЦИЯ]** Заголовок `decisions.md:7` фиксирует «Версия приложения на момент ревизии: 0.7.2», хотя нижние записи файла (включая ADR-019) документируют работу вплоть до 0.9.1 — по собственному правилу файла («если целевая архитектура расходится с реализацией — укажи drift явно», `decisions.md:554`) этот заголовок, вероятно, следует обновить | `decisions.md:7,554` | Владелец `decisions.md`; вне мандата этого SPEC (файл не редактировался по ограничению задания) |
