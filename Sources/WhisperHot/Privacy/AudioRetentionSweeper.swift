...
enum AudioRetentionSweeper {
        ...
        while let url = enumerator.nextObject() as? URL {
            ...
            if age >= maxAge {
                do {
                    ...
                    NSLog("WhisperHot: failed to delete stale audio \(url.lastPathComponent) → \(error.localizedDescription)")
                }
            }
        }
        if deleted > 0 {
            NSLog("WhisperHot: retention sweep removed \(deleted) audio file(s)")
        }
    }
    ...
    static func wipeAll(includingActive: Bool = false) {
        ...
        for url in contents {
            ...
            do {
                ...
                NSLog("WhisperHot: failed to wipe \(url.lastPathComponent) → \(error.localizedDescription)")
            }
        }
        if deleted > 0 {
            NSLog("WhisperHot: retention wipe removed \(deleted) audio file(s)")
        }
    }
    ...
    static func delete(_ url: URL) {
        ...
        do {
            ...
            NSLog("WhisperHot: failed to delete \(url.lastPathComponent) → \(error.localizedDescription)")
        }
    }
}
