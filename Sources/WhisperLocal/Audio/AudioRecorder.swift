...
final class AudioRecorder: NSObject {
    ...
    private let writerQueue = DispatchQueue(
        label: "com.aleksejsupilin.WhisperLocal.audio.writer",
        qos: .userInitiated
    )
    ...
    private func processTapBuffer(_ buffer: AVAudioPCMBuffer) {
        ...
        guard status != .error, error == nil else {
            NSLog("WhisperLocal: converter error: \(error?.localizedDescription ?? "unknown")")
            return
        }
        ...
        writerQueue.async {
            do {
                ...
                NSLog("WhisperLocal: audio write error: \(error.localizedDescription)")
            }
        }
    }
    ...
    private func installConfigurationObserver() {
        configurationObserver = NotificationCenter.default.addObserver(
            ...
            NSLog("WhisperLocal: audio configuration changed; stopping recording")
